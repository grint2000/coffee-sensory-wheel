...
async function scenario(failInstall = false) {
  ...
  await caches.open('noel-sca-app-v8'); await caches.open('noel-sca-runtime-v9'); await caches.open('noel-sca-app-v10'); await caches.open('noel-sca-app-v11'); await caches.open('unrelated-user-cache');
  await wait('install'); assert(skip); await wait('activate'); assert(claim);
  assert.deepEqual((await caches.keys()).sort(),['noel-sca-app-v12','unrelated-user-cache'].sort());
...
(async()=>{await scenario();await scenario(true);console.log('PASS: install, scoped v8/v9/v10/v11 cleanup, offline dictionary/page, runtime fallback, failed-update protection. Mock Cache API; physical browser migration remains separate.');})().catch(error=>{console.error(error);process.exitCode=1;});
